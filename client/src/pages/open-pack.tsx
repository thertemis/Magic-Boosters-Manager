...
import { ArrowLeft, Sparkles, Layers, ExternalLink, X, Library, DollarSign, Coins } from "lucide-react";
...
export default function OpenPackPage() {
  ...
  const collection = useQuery<{ cardId: string; quantity: number; card: { name: string } }[]>({
    queryKey: ["/api/player/collection"],
    staleTime: 60000,
  });

  const balance = useQuery<{ balance: number; settings: { currencyName: string; currencySymbol: string; sellRateMultiplier: number; economyEnabled: boolean } | null }>({
          ...
          >
            ...
            >
              ...
              <div className="mt-4 text-center space-y-2">
                ...
                {/* Card stats: owned, price, currency value */}
                <div className="flex items-center justify-center gap-4 mt-3 flex-wrap">
                  <div className="flex items-center gap-1.5 text-sm">
                    <Library className="w-4 h-4 text-blue-400" />
                    <span className="text-muted-foreground">Owned:</span>
                    <span className="font-bold text-white">{ownedByName.get(enlargedCard.card.name) || 0}</span>
                  </div>
                  {(() => {
                    const prices = (enlargedCard.card.prices as any) || {};
                    const usd = parseFloat(
                      (enlargedCard.isFoil ? (prices.usd_foil || prices.usd) : prices.usd) || "0"
                    );
                    const econ = balance.data?.settings;
                    const currencyVal = econ?.economyEnabled && usd > 0
                      ? Math.floor(usd * (econ.sellRateMultiplier ?? 50))
                      : null;
                    return (<>
                      <div className="flex items-center gap-1.5 text-sm">
                        <DollarSign className="w-4 h-4 text-green-400" />
                        <span className="text-muted-foreground">Price:</span>
                        <span className="font-bold text-white">{usd > 0 ? `$${usd.toFixed(2)}` : "—"}</span>
                      </div>
                      {currencyVal !== null && (
                        <div className="flex items-center gap-1.5 text-sm">
                          <Coins className="w-4 h-4 text-amber-400" />
                          <span className="font-bold text-amber-300">
                            {currencyVal} {econ!.currencySymbol}
                          </span>
                        </div>
                      )}
                    </>);
